...
function fetchItemThreads(state) {
  const type = state.router.location.pathname.replace('/',''); // remove the first slash
  const page = parseInt(state.router.location.query.p) || 1;
  return dispatch => {
    ...
    return type === 'newcomments' ?
      ...
      storiesRef(getApiUrlByItemType(type)).once('value', function(snapshot) {
        const skim = snapshot.val().slice((page - 1) * MAX_THREAD_NUMBER, MAX_THREAD_NUMBER * page); // reduce the page size
